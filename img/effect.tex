...
\draw[force,->] (1.5,1.5)  -- ++(0,0.7) node[above] {$\vec{F}_\text{л}$};% {$\vec{F}_\text{л}$};
\draw[force,->] (1.5,1.5)  -- ++(0,-0.7) node[below] {$\vec{F}_\text{к}$};% {$\vec{F}_\text{л}$};
...
%
% \draw[axis, <->] (-0.4,0) -- node[left,black] {$b$} ++ (0,3);
